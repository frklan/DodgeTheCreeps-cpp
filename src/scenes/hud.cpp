...
namespace godot {
  ...
  void HUD::_ready() {
    get_node("StartButton")->connect("pressed", this, "onStartNewGamePressed");
    get_node("MessageTimer")->connect("timeout", this, "onMessageTimerTimeout");
  ...
  void HUD::onStartNewGamePressed() {
    cast_to<Button>(get_node("StartButton"))->hide();
  ...
  void HUD::onGameOver() {
    ...
    cast_to<Button>(get_node("StartButton"))->show();
  }

  void HUD::onMessageTimerTimeout() {
    cast_to<Label>(get_node("MessageLabel"))->hide();
  }

  void HUD::showMessage(godot::String message, bool timeout) {
    auto msgLabel = cast_to<Label>(get_node("MessageLabel"));
    msgLabel->set_text(message);
    msgLabel->show();

    if(timeout == true) {
      auto timer = cast_to<Timer>(get_node("MessageTimer"));
      timer->start();
    }
  }

  void HUD::showScore(const int score) {
    auto scoreLabel =  cast_to<Label>(get_node("ScoreLabel"));
